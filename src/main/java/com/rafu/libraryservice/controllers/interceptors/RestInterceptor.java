...
public class RestInterceptor implements AsyncHandlerInterceptor {
  ...
  private boolean isCreateUserRequest(HttpServletRequest request) {
    return request.getRequestURI().contains(USERS.getValue())
        && Objects.equals(request.getMethod(), POST_METHOD.getValue());
  }

  private boolean isLoginRequest(HttpServletRequest request) {
    return request.getRequestURI().contains(LOGIN.getValue())
        && Objects.equals(request.getMethod(), POST_METHOD.getValue());
  }

  private boolean isSwaggerRequest(HttpServletRequest request) {
    return (request.getRequestURI().contains(SWAGGER_UI.getValue())
            || request.getRequestURI().contains(API_DOCS.getValue()))
